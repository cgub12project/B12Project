/**
 * ThreadDetailActivity.kt — 對話詳情頁面
 *
 * 所屬模組：ui/detail（詳情模組）
 *
 * 本 Activity 展示特定訊息警報的完整對話分析結果，功能包含：
 * - 標頭資訊：來源 App、群組名稱、傳送者名稱
 * - 風險膠囊按鈕（RiskPill）：顯示風險分數，點擊可導航至 [AccountDetailActivity]
 * - 摘要統計：可疑訊息數、總訊息數、接觸天數
 * - 詐騙模式標籤（Pattern Tags）：AI 偵測到的詐騙行為模式
 * - 階段式聊天列表：以 [ChatAdapter] 呈現分階段的對話記錄與 AI 風險標記
 *
 * 資料來源為 [SampleData.getThreads()]，透過 Intent 傳入 threadId 查詢。
 */
...
/**
 * 對話詳情 Activity，展示階段式聊天記錄與 AI 風險分析結果。
 */
